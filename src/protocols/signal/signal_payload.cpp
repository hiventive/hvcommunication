...
SignalPayload::SignalPayload(MemoryManagementIf* mm_) :
		BasePayload(mm_), mSignalValue(false), mResponseStatus(
				SIGNAL_OK_RESPONSE) {
}
...
// Accessors
bool SignalPayload::getValue() const {
	return mSignalValue;
}
...
void SignalPayload::setResponseStatus(
		const SignalResponseStatus& responseStatus_) {
